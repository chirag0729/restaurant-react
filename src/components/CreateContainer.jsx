...
//               animate={{ opacity: 1 }}
//               exit={{ opacity: 0 }}
//               className={`w-full p-2 rounded-lg text-center text-lg font-semibold ${alertStatus === "danger" ? "bg-red-400 text-red-700" : "bg-emerald-400 text-emerald-800"}`}>{msg}</motion.p>
//           )}
...
//           <select onChange={(e) => setCategory(e.target.value)} className="outline-none w-full text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer bg-gray-200">
//             <option value="other" className='bg-white'>Select Category</option>
//             {categories && categories.map((item) => (
//               <option key={item.id} className="text-base border-0 outline-none capitalize bg-gray-100 text-headingColor" value={item.urlParamName}>{item.name}</option>
//             ))}
//           </select>
//         </div>
//         <div className='group flex justify-center items-center flex-col border-2 border-dotted border-gray-300 w-full h-225 md:h-420 cursor-pointer rounded-lg'>
//           {isLoading ? (<Loader />) : (<> {!imageAsset ? (
//             <>
